...
public class GridLabelRenderer {
	...
	public final class Styles {
		/**
		 * the general text size of the axis titles. can be overwritten with
		 * #verticalAxisTitleTextSize and #horizontalAxisTitleTextSize
		 */
		...
		/**
		 * flag whether the zero-lines (vertical+ horizontal) shall be
		 * highlighted
		 */
	 ...
	 * Pixel (y) Value = y-value
	 */
	private Map<Integer, Double> mStepsVertical;

	/**
	 * cache of the vertical steps for the second scale, which is on the right
	 * side (horizontal lines and vertical labels) Key = Pixel (y) Value =
	 * y-value
	 */
	private Map<Integer, Double> mStepsVerticalSecondScale;

	/**
	 * cache of the horizontal steps (vertical lines and horizontal labels) Key
	 * = Pixel (x) Value = x-value
	 */
	...
	/**
	 * flag whether is bounds are automatically adjusted for nice human-readable
	 * numbers
	 */
	 ...
	 * 
	 * @param graphView
	 *            the corresponding graphview object
	 */
	...
	/**
	 * resets the styles. This loads the style from reading the values of the
	 * current theme.
	 */
	public void resetStyles() {
		...
		mGraphView
				.getContext()
				.getTheme()
				.resolveAttribute(android.R.attr.textAppearanceSmall,
						typedValue, true);
		...
		try {
			...
			size = array.getDimensionPixelSize(2, 5);
			size2 = array.getDimensionPixelSize(3, 5);
			array.recycle();
		} catch (Exception e) {
			color1 = Color.BLACK;
			color2 = Color.GRAY;
	...
	/**
	 * will load the styles to the internal paint objects (color, text size,
	 * text align)
	 */
	 ...
	 * Normally you should always call
	 * {@link GraphView#onDataChanged(boolean, boolean)} which will call this
	 * method.
	 * 
	 * @param keepLabelsSize
	 *            true if you don't want to recalculate the size of the labels.
	 *            It is recommended to use "true" because this will improve
	 *            performance and prevent a flickering.
	 * @param keepViewport
	 *            true if you don't want that the viewport will be recalculated.
	...
	/**
	 * calculates the vertical steps of the second scale. This will not do any
	 * automatically update of the bounds. Use always manual bounds for the
	 * second scale.
	 * 
	...
	protected boolean adjustVerticalSecondScale() {
		...
		if (mStepsVerticalSecondScale != null) {
			...
			mStepsVerticalSecondScale = new LinkedHashMap<Integer, Double>(
					numVerticalLabels);
		}
	...
	/**
	 * calculates the vertical steps. This will automatically change the bounds
	 * to nice human-readable min/max.
	 * 
	 * @return true if it is ready
	 */
	protected boolean adjustVertical() {
		...
		if (!mGraphView.getViewport().isYAxisBoundsManual()) {
			mGraphView.getViewport().setYAxisBoundsStatus(
		...
		if (mStepsVertical != null) {
			...
			mStepsVertical = new LinkedHashMap<Integer, Double>(
					numVerticalLabels);
		}
		int height = mGraphView.getGraphContentHeight();
		double v = newMaxY;
	...
	/**
	 * calculates the horizontal steps. This will automatically change the
	...
	protected boolean adjustHorizontal() {
		...
		if (mGraphView.getViewport().isXAxisBoundsManual()
				&& mGraphView.getViewport().getXAxisBoundsStatus() != Viewport.AxisBoundsStatus.READJUST_AFTER_SCALE) {
			// scaling
			if (mGraphView.getViewport().mScalingActive) {
				...
				// numHorizontalLabels *=
				// (mGraphView.getViewport().mCurrentViewport.width()+oldStep)/(mGraphView.getViewport().mScalingBeginWidth+oldStep);
			...
			while (adjusting) {
				...
				if (mGraphView.getViewport().getXAxisBoundsStatus() == Viewport.AxisBoundsStatus.READJUST_AFTER_SCALE) {
					...
					if (mGraphView.getViewport().mCurrentViewport.width() < mGraphView
							.getViewport().mScalingBeginWidth) {
						roundAlwaysUp = false;
					}
				}
			...
			if (mGraphView.getViewport().getXAxisBoundsStatus() == Viewport.AxisBoundsStatus.READJUST_AFTER_SCALE) {
				mGraphView.getViewport().setXAxisBoundsStatus(
						Viewport.AxisBoundsStatus.FIX);
			} else {
				mGraphView.getViewport().setXAxisBoundsStatus(
						Viewport.AxisBoundsStatus.AUTO_ADJUSTED);
			}
		...
		if (mStepsHorizontal != null) {
			...
			mStepsHorizontal = new LinkedHashMap<Integer, Double>(
		...
		double v = newMinX;
		double p = mGraphView.getGraphContentLeft(); // start
		double pixelStep = width / (numHorizontalLabels - 1);

		if (mGraphView.getViewport().mScalingActive) {
			double oldStep = mGraphView.getViewport().mScalingBeginWidth
					/ (numHorizontalLabels - 1);
			double factor = (mGraphView.getViewport().mCurrentViewport.width() + oldStep)
					/ (mGraphView.getViewport().mScalingBeginWidth + oldStep);
			pixelStep *= 1f / factor;
			...
			// scrolled = ((float) mGraphView.getViewport().getMinX(false) -
			// mGraphView.getViewport().mScalingBeginLeft)*2;
			double newWidth = width * 1f / factor;
			scrolledPixels = (newWidth - width) * -0.5f;

		}

		// scrolling
		if (!Double.isNaN(mGraphView.getViewport().mScrollingReferenceX)) {
			scrolled = mGraphView.getViewport().mScrollingReferenceX - newMinX;
			scrolledPixels += scrolled * (pixelStep / exactSteps);
	...
	/**
	 * adjusts the grid and labels to match to the data this will automatically
	 * change the bounds to nice human-readable values, except the bounds are
	 * manual.
	 */
	...
	protected void calcLabelVerticalSize(Canvas canvas) {
		...
		String testLabel = mLabelFormatter.formatLabel(mGraphView.getViewport()
				.getMaxY(false), false);
		if (testLabel == null)
			testLabel = "";
		...
		testLabel = mLabelFormatter.formatLabel(mGraphView.getViewport()
				.getMinY(false), false);
		if (testLabel == null)
			testLabel = "";
	...
	protected void calcLabelVerticalSecondScaleSize(Canvas canvas) {
		...
		// test label
		double testY = ((mGraphView.mSecondScale.getMaxY() - mGraphView.mSecondScale
				.getMinY()) * 0.783) + mGraphView.mSecondScale.getMinY();
		String testLabel = mGraphView.mSecondScale.getLabelFormatter()
				.formatLabel(testY, false);
		Rect textBounds = new Rect();
		mPaintLabel.getTextBounds(testLabel, 0, testLabel.length(), textBounds);
	...
	protected void calcLabelHorizontalSize(Canvas canvas) {
		// test label
		double testX = ((mGraphView.getViewport().getMaxX(false) - mGraphView
				.getViewport().getMinX(false)) * 0.783)
		...
		if (!mLabelHorizontalHeightFixed) {
			...
			mLabelHorizontalHeight = (int) Math.max(mLabelHorizontalHeight,
					mStyles.textSize);
		}
	...
	protected void drawHorizontalSteps(Canvas canvas) {
		...
		for (Map.Entry<Integer, Double> e : mStepsHorizontal.entrySet()) {
			...
			if (mStyles.gridStyle.drawVertical()) {
				canvas.drawLine(e.getKey(), mGraphView.getGraphContentTop(),
	...
	protected void drawVerticalStepsSecondScale(Canvas canvas) {
		...
		for (Map.Entry<Integer, Double> e : mStepsVerticalSecondScale
				.entrySet()) {
			// draw label
			...
			String[] lines = mGraphView.mSecondScale.mLabelFormatter
					.formatLabel(e.getValue(), false).split("\n");
			y += (lines.length * getTextSize() * 1.1f) / 2; // center text
															// vertically
			for (int li = 0; li < lines.length; li++) {
				// for the last line y = height
	...
	protected void drawVerticalSteps(Canvas canvas) {
		...
		for (Map.Entry<Integer, Double> e : mStepsVertical.entrySet()) {
			...
			if (mStyles.gridStyle.drawHorizontal()) {
				canvas.drawLine(startLeft, e.getKey(),
						startLeft + mGraphView.getGraphContentWidth(),
						e.getKey(), mPaintLine);
			}
			...
			if (isVerticalLabelsVisible()) {
				...
				for (int li = 0; li < lines.length; li++) {
					// for the last line y = height
					float y2 = y - (lines.length - li - 1) * getTextSize()
							* 1.1f;
					canvas.drawText(lines[li], labelsOffset, y2, mPaintLabel);
				}
			}
	 ...
	 * 
	 * @param in
	 *            the raw value that is to be rounded
	 * @param roundAlwaysUp
	 *            true if it shall always round up (ceil)
	...
	public int getLabelVerticalWidth() {
		return mLabelVerticalWidth == null || !isVerticalLabelsVisible() ? 0
				: mLabelVerticalWidth;
	}

	/**
	 * sets a manual and fixed with of the space for the vertical labels. This
	 * will prevent GraphView to calculate the width automatically.
	 * 
	 * @param width
	 *            the width of the space for the vertical labels. Use null to
	 *            let GraphView automatically calculate the width.
	 */
	...
	public int getLabelHorizontalHeight() {
		return mLabelHorizontalHeight == null || !isHorizontalLabelsVisible() ? 0
				: mLabelHorizontalHeight;
	}

	/**
	 * sets a manual and fixed height of the space for the horizontal labels.
	 * This will prevent GraphView to calculate the height automatically.
	 * 
	 * @param height
	 *            the height of the space for the horizontal labels. Use null to
	 *            let GraphView automatically calculate the height.
	 */
	...
	/**
	 * @param textSize
	 *            the general text size of the axis titles. can be overwritten
	 *            with {@link #setVerticalAxisTitleTextSize(float)} and
	 *            {@link #setHorizontalAxisTitleTextSize(float)}
	 */
	...
	/**
	 * @param verticalAxisTitleTextSize
	 *            font size of the vertical axis title
	 */
	...
	/**
	 * @param horizontalAxisTitleTextSize
	 *            font size of the horizontal axis title
	 */
	...
	/**
	 * @param horizontalAxisTitleColor
	 *            font color of the horizontal axis title
	 */
	...
	/**
	 * @param verticalLabelsSecondScaleAlign
	 *            the alignment of the labels on the right side
	 */
	public void setVerticalLabelsSecondScaleAlign(
			Paint.Align verticalLabelsSecondScaleAlign) {
	...
	/**
	 * @param verticalLabelsSecondScaleColor
	 *            the color of the labels on the right side
	 */
	public void setVerticalLabelsSecondScaleColor(
			int verticalLabelsSecondScaleColor) {
	...
	public int getLabelVerticalSecondScaleWidth() {
		return mLabelVerticalSecondScaleWidth == null ? 0
				: mLabelVerticalSecondScaleWidth;
	}
	...
	/**
	 * @param mNumHorizontalLabels
	 *            count of the horizontal labels, that will be shown at one
	 *            time.
	 */
	...
	/**
	 * 
	 * 控制线框 横向 纵向 双重显示 Define which grid lines shall be drawn
	 * 
	 * @param gridStyle
	 *            the grid style
	 */
