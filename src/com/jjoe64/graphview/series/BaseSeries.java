...
/**
 * Basis implementation for series. Used for series that are plotted on a
 * default x/y 2d viewport.
 * 
 * Extend this class to implement your own custom graph type.
 * 
 * This implementation uses a internal Array to store the data. If you want to
 * implement a custom data provider you may want to implement
 * {@link com.jjoe64.graphview.series.Series}.
 * 
 * @author jjoe64
 */
public abstract class BaseSeries<E extends DataPointInterface> implements
		Series<E> {
	/**
	 * holds the data
	 */
	 ...
	 * 
	 * will be filled while drawing via
	 * {@link #registerDataPoint(float, float, DataPointInterface)}
	 */
	 ...
	 * 
	 * @param data
	 *            data points important: array has to be sorted from lowest
	 *            x-value to the highest
	 */
	...
	/**
	 * get the values for a given x range. if from and until are bigger or equal
	 * than all the data, the original data is returned. If it is only a part of
	 * the data, the range is returned plus one datapoint before and after to
	 * get a nice scrolling.
	 * 
	 * @param from
	 *            minimal x-value
	 * @param until
	...
	/**
	 * set the color of the series. This will be used in plotting (depends on
	 * the series implementation) and is used in the legend.
	 * 
	 * @param mColor
	 */
	...
	/**
	 * called by the tap detector in order to trigger the on tap on datapoint
	 * event.
	 * 
	 * @param x
	 *            pixel
	 ...
	 * 
	 * @param onlyLast
	 *            if not null, it will only check that this datapoint is after
	 *            the last point.
	 */
