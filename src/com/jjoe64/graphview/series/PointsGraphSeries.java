...
/**
 * Series that plots the data as points. The points can be different shapes or a
 * complete custom drawing.
 * 
 * @author jjoe64
 */
public class PointsGraphSeries<E extends DataPointInterface> extends
		BaseSeries<E> {
	/**
	...
	public static interface CustomShape {
		/**
		 * called when drawing a single data point. use the x and y coordinates
		 * to render your drawing at this point.
		 * 
		 * @param canvas
		 *            canvas to draw on
		 * @param paint
		 *            internal paint object. this has the correct color. But you
		 *            can use your own paint.
		 * @param x
		 *            x-coordinate the point has to be drawn to
		 * @param y
		 *            y-coordinate the point has to be drawn to
		 * @param dataPoint
		 *            the related data point
		 */
		void draw(Canvas canvas, Paint paint, double x, double y,
				DataPointInterface dataPoint);
	}

	/**
	 * choose a predefined shape to render for each data point. You can also
	 * render a custom drawing via
	 * {@link com.jjoe64.graphview.series.PointsGraphSeries.CustomShape}
	 */
	public enum Shape {
		/**
	...
	private final class Styles {
		/**
		 * this is used for the size of the shape that will be drawn. This is
		 * useless if you are using a custom shape.
		 */
		double size;
	 ...
	 *            canvas to draw on
	 * @param isSecondScale
	 *            whether it is the second scale
	 */
	@Override
			...
			if (!overdraw) {
				if (mCustomShape != null) {
					...
					canvas.drawCircle((float) endX, (float) endY,
							(float) mStyles.size, mPaint);
				} else if (mStyles.shape == Shape.RECTANGLE) {
					canvas.drawRect((float) endX - (float) mStyles.size,
							(float) endY - (float) mStyles.size, (float) endX
									+ (float) mStyles.size, (float) endY
									+ (float) mStyles.size, mPaint);
				} else if (mStyles.shape == Shape.TRIANGLE) {
					Point[] points = new Point[3];
					points[0] = new Point((int) endX, (int) (endY - getSize()));
					points[1] = new Point((int) (endX + getSize()),
							(int) (endY + getSize() * 0.67));
					points[2] = new Point((int) (endX - getSize()),
							(int) (endY + getSize() * 0.67));
	...
	private void drawArrows(Point[] point, Canvas canvas, Paint paint) {
		...
		canvas.drawVertices(Canvas.VertexMode.TRIANGLES, 8, points, 0, null, 0,
				null, 0, null, 0, 0, paint);
		Path path = new Path();
